        ...
        <div class="nav-item dropdown">
          ...
          >
            <span class="bg-primary text-white avatar rounded-circle">
              {{ getInitials(authStore.user?.username || "User Name") }}
            </span>
            <div class="d-none d-xl-block ps-2">
              <div class="fw-bold">{{ authStore.user?.username?.toUpperCase() || 'USER NAME' }}</div>
              <div class="mt-1 small text-primary">{{ authStore.user?.role?.toUpperCase() || 'ROLE TIDAK DIKETAHUI' }}</div>
            </div>
          </a>
